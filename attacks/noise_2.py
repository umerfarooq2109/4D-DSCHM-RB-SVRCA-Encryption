"""
Visual Color (RGB) Noise Attack Figure Generator for 4D Chaos CA Cryptosystem (5 Noise Levels).

Generates publication-quality figures showing the encrypted images corrupted
by varying levels of noise (1%, 2%, 3%, 4%, 5% intensity) and their corresponding
decrypted color results side-by-side, matching the style of standard journal publications.

Saved in results/Noise folder with the name nameoftheimage_noise_2.png at 600 DPI.
"""
...
def generate_noise_figure(image_path="Images/pepper.tiff"):
    """
    Apply color salt & pepper noise with varying densities (0.01 to 0.05), decrypt, and generate a 2x5 grid:
    ...
    # Define the 5 noise intensities: 1%, 2%, 3%, 4%, 5%
    scenarios = [
        ...
        {"name": "4% Noise Intensity", "density": 0.04, "label": "(d) 4% Intensity"},
        {"name": "5% Noise Intensity", "density": 0.05, "label": "(e) 5% Intensity"}
    ]
    ...
    # Setup the plot with tight vertical space (5 columns)
    ...
    save_path = os.path.join(output_dir, f"{base_name}_noise_2.png")
